...
class ContourDetector:
    ...
    def find_card_contour_candidates_from_image(
        ...
        relaxed_contour_metrics: dict[str, object] | None = None
        for contour in contours:
            if self.is_valid_card_contour(contour, resized_image.shape):
                ...
                relaxed_contour_metrics = None
                break

            # Ảnh chụp sát mép có thể làm một hoặc hai góc thẻ nằm ngoài
            # khung. Khi đó minAreaRect của contour bị cụt có tỷ lệ < 1.35
            # dù contour sáng vẫn là gần trọn CCCD. Chỉ nhận fallback này
            # khi vùng sáng rất lớn, gần hình chữ nhật và chi tiết trải đều
            # toàn quad; ba điều kiện cùng lúc loại nền/tay dính threshold và
            # các hình chữ nhật nội bộ như QR, chân dung hay một dải chữ.
            if relaxed_contour_metrics is not None:
                continue
            ...
            candidate_quad = self.contour_to_quadrilateral(
                contour,
                resized_image.shape,
            )
            candidate_metrics = self._score_edge_quadrilateral(
                candidate_quad,
                edge_map,
            )
            ...
            candidate_area_ratio = float(
                candidate_metrics.get("areaRatio") or 0.0
            )
        ...
        contour_is_complete = bool(
            contour_quad is not None
            and (
                relaxed_contour_metrics is not None
                or
        ...
        if contour_is_complete:
            hough_skipped_reason = (
                "LARGE_FOREGROUND_CONTOUR_COMPLETE"
                if relaxed_contour_metrics is not None
                else "PRIMARY_CONTOUR_COMPLETE"
            )
        else:
        ...
        method = (
            "large_foreground_contour"
            if relaxed_contour_metrics is not None
            else "brightness_contour"
        )
        ...
        return primary, mask, contours, {
            ...
            "relaxedForegroundContour": relaxed_contour_metrics,
